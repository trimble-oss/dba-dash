...
SetSchedule - Set how often a collection runs.  Sets the service level schedule, or a per-connection override when -c/--ConnectionID is supplied.  Use --CollectionType with --Schedule, --RunOnServiceStart and/or --ScheduleClear.")]
        ...
        [Option("NoDeadlockBackfill", Default = false, Required = false, HelpText = "Default is to read system_health on the first deadlock collection as well as the configured session, so a dedicated session that starts empty still collects deadlocks from before it existed.  The backfill is read once.  Use this switch to disable.  No effect when the session is system_health, or on Azure SQL Database.")]
