...
builder.Services.AddScoped<OpenCodeChatClient>();
    ...
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        ...
        if (request.IncludeAiSummary)
        {
            ...
            response.Summary = await openCode.SummarizeWithPromptAsync(prompt, cancellationToken);
        }
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var summary = await openCode.SummarizeWithPromptAsync(summaryPrompt, cancellationToken);
...
static void LoadSettingsFromServiceConfig(ConfigurationManager config)
{
    ...
    config["OpenCode:BaseUrl"] = serviceConfig.OpenCodeBaseUrl ?? string.Empty;
    config["OpenCode:ApiKey"] = serviceConfig.OpenCodeApiKeyDecrypted ?? string.Empty;
    config["OpenCode:Model"] = serviceConfig.OpenCodeModel ?? string.Empty;
}
