...
namespace DBADash.QueryPlan.Layout
{
    ...
    ///
    /// The deadlock viewer defines the same three value types for the same reasons.  They are
    /// deliberately not shared: each viewer is a self contained, independently portable unit, and a
    /// shared geometry assembly would couple them for the sake of ninety lines of arithmetic that
    /// has not changed since it was written.  If a third viewer arrives, extract then.
    /// </summary>
    ...
    public readonly struct LayoutRect : IEquatable<LayoutRect>
    {
        ...
        /// <summary>The middle of the left edge - where an arrow into this node lands.</summary>
        public LayoutPoint LeftCentre => new(X, Y + (Height / 2));

        /// <summary>The middle of the right edge - where an arrow out of this node starts.</summary>
        public LayoutPoint RightCentre => new(Right, Y + (Height / 2));
        ...
        /// <summary>True when the two rectangles share any area, used to cull off-screen drawing.</summary>
