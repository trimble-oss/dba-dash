...
OUTER APPLY(SELECT CASE WHEN DATEDIFF(mi,J.LastFailed,GETUTCDATE())  <= cfg.TimeSinceLastFailureCritical THEN 1 WHEN DATEDIFF(mi,J.LastFailed,GETUTCDATE()) <= cfg.TimeSinceLastFailureCritical THEN 2 WHEN cfg.TimeSinceLastFailureWarning IS NULL AND cfg.TimeSinceLastFailureCritical IS NULL THEN 3 ELSE 4 END AS TimeSinceLastFailureStatus,
				CASE WHEN DATEDIFF(mi,J.LastSucceeded,GETUTCDATE())  >= cfg.TimeSinceLastSucceededCritical THEN 1 WHEN DATEDIFF(mi,J.LastSucceeded,GETUTCDATE()) >= cfg.TimeSinceLastSucceededWarning THEN 2 WHEN cfg.TimeSinceLastSucceededWarning IS NULL AND cfg.TimeSinceLastSucceededCritical IS NULL THEN 3 ELSE 4 END AS TimeSinceLastSucceededStatus,
